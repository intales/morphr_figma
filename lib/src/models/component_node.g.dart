...
abstract class _$ComponentNodeCWProxy {
  ...
  ComponentNode size(Vector2D? size);
  ...
  ComponentNode absoluteBoundingBox(SizeRectangle? absoluteBoundingBox);

  ComponentNode absoluteRenderBounds(SizeRectangle? absoluteRenderBounds);

  ComponentNode rotation(double? rotation);

  ComponentNode name(String? name);

  ComponentNode pluginData(dynamic pluginData);
  ...
  ComponentNode strokeAlign(StrokeAlign? strokeAlign);

  ComponentNode cornerRadius(double? cornerRadius);

  ComponentNode rectangleCornerRadii(List<double>? rectangleCornerRadii);
  ...
  ComponentNode layoutAlign(LayoutAlign? layoutAlign);

  ComponentNode transitionNodeID(String? transitionNodeID);

  ComponentNode transitionDuration(double? transitionDuration);

  ComponentNode relativeTransform(List<List<double>>? relativeTransform);

  ComponentNode clipsContent(bool? clipsContent);

  ComponentNode layoutMode(LayoutMode? layoutMode);
  ...
  /// Usage
  /// ```dart
  /// ComponentNode(...).copyWith(id: 12, name: "My name")
  /// ````
  ComponentNode call({
    String id,
    bool visible,
...
class _$ComponentNodeCWProxyImpl implements _$ComponentNodeCWProxy {
  ...
  @override
  ComponentNode size(Vector2D? size) => this(size: size);
  ...
  @override
  ComponentNode rotation(double? rotation) => this(rotation: rotation);

  @override
  ComponentNode name(String? name) => this(name: name);

  @override
  ComponentNode pluginData(dynamic pluginData) => this(pluginData: pluginData);
  ...
  @override
  ComponentNode individualStrokeWeights(
          StrokeWeights? individualStrokeWeights) =>
  ...
  ComponentNode call({
    ...
    Object? horizontalPadding = const $CopyWithPlaceholder(),
    Object? verticalPadding = const $CopyWithPlaceholder(),
    ...
    return ComponentNode(
      ...
      layoutMode: layoutMode == const $CopyWithPlaceholder()
          ? _value.layoutMode
          // ignore: cast_nullable_to_non_nullable
